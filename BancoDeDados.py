...
class BancoDeDados():
    ...
    def criar_tabelas(self):
        ...
        self.cursor.execute("""
            CREATE TABLE IF NOT EXISTS NUMEROS_TELEFONE (
                ...
                FOREIGN KEY(cpf_do_cliente) REFERENCES USUARIO(cpf),
    ...
    def adicionar_numero(self, numero, cpf_do_cliente, id_assinatura):
        try:
            self.cursor.execute("""
                INSERT INTO NUMEROS_TELEFONE (numero, cpf_do_cliente, id_assinatura)
                VALUES (?, ?, ?)
            """, (numero, cpf_do_cliente, id_assinatura))
            self.conn.commit()
        except sqlite3.IntegrityError as e:
            print(f"[ERRO NÚMERO] {e}")
...
print(f"\nO email filipe@gmail.com existe? {bd.email_existe('filipe@gmail.com')}")
print(f"O email filipeihancis@gmail.com existe? {bd.email_existe('filipeihancis@gmail.com')}")
>>>>>>> 6b3fea13cf723d067b39576e563c5ec99a1690eb
